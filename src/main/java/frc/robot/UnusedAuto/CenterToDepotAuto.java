...
public class CenterToDepotAuto extends SequentialCommandGroup {
  public CenterToDepotAuto (SwerveSubsystem drive) {
    final double[] startYawRad = new double[1];
      addCommands(
        drive.startAutoAt(4.61, 4.03, 90.0),
        ...
        //Turn ~90 degrees in place (robot-centric)
        Commands.runOnce(()->startYawRad[0] = drive.getYaw().getRadians(), drive),
        Commands.run(()->{
          double targetYawRad = startYawRad[0] + (Math.PI / 2.0);
        ...
        // Move forward ~1m (0.5 m/s for 2s) after turning (to the depot)
